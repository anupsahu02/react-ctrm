...
app.post("/api/authenticate", (req, res) => {
    if (req.body.email && req.body.password) {
        var options = {
            method: "POST",
            url: "http://reference.qa2.ekaanalytics.com:2390/cac-security/api/oauth/token",
            qs: { client_id: "2", grant_type: "cloud_credentials" },
            headers: {
                Authorization: "Basic " + base64.encode(req.body.email + ":" + req.body.password),
                "cache-control": "no-cache",
                Accept: "application/json"
            }
        };
...
function tokenValidation(token, res) {
    var options = {
        method: "POST",
        url: "http://reference.qa2.ekaanalytics.com:2390/cac-security/api/oauth/validate_token",
        qs: {
            deviceIdentifier: "1",
